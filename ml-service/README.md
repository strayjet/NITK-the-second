...
```
YOLO Detection Service (ml-service)   <-- this repo
        ↓
Event Aggregator (backend)             <-- CityMind/backend, HTTP client of this service
        ↓
Traffic Simulation / Infrastructure Planning   <-- not yet built
        ↓
AI Copilot                                       <-- not yet built
        ↓
React Dashboard                                    <-- not yet built
```

See the top-level `CityMind/README.md` for how this fits together with `backend/` and how to run both services at once.
